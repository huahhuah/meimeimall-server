package cn.tedu.meimall.front.account.filter;
...
import cn.tedu.meimall.front.account.dao.cache.IUserCacheRepository;
...
import static cn.tedu.meimall.common.consts.web.HttpConsts.HEADER_AUTHORIZATION;
import static cn.tedu.meimall.common.consts.web.HttpConsts.HEADER_USER_AGENT;
import static cn.tedu.meimall.common.consts.web.JwtConsts.*;
 ...
 *      嘗試接收客戶端的請求中攜帶的JWT資料
 *      嘗試解析JWT資料
 *      將解析得到的用戶資料創建為Authentication對象,並存入到SecurityContext中
 *
 */
@Slf4j
@Component
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    public JwtAuthorizationFilter(){
        log.info("創建過濾器對像: JwtAuthorizationFilter");
    }

    @Override
    protected void doFilterInternal(HttpServletRequest request, @org.jetbrains.annotations.NotNull HttpServletResponse response, @NotNull FilterChain filterChain) throws ServletException, IOException {
        log.debug("處理JWT的過濾器開始處理當前請求...");
        //直接放行登入和註冊的請求
        String[] permitUrls = {"/users/register"};
        String requestURI = request.getRequestURI();
        log.debug("請求URL: {}", requestURI);
        ...
        try{
            ...
            String message = "服務器忙,請稍後再試!【看到這句時,你應該檢查服務器端的控制台,並在JwtAuthorizationFilter中解析JWT時添加處理異常的catch程式碼】";
        ...
        if(!userAgent.equals(request.getHeader(HEADER_USER_AGENT))
            ...
            log.warn("本次請求的信息與當初登入時完全不同,將直接放行,交由後續組件處理...");
            filterChain.doFilter(request, response);
            return;
        }
        ...
        log.debug("從Redis中讀取當前用戶的權限 = {}", authoritiesJsonString);
        ...
        //過濾器鏈繼續執行,即: 放行
        log.debug("驗證JWT完畢,已經向SecurityContext中存入認證信息,過濾器放行");
        filterChain.doFilter(request, response);
