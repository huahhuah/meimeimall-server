package cn.tedu.meimall.front.mall.filter;
...
import cn.tedu.meimall.front.mall.dao.cache.IUserCacheRepository;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter implements HttpConsts, JwtConsts {
    ...
    protected void doFilterInternal(HttpServletRequest request, @org.jetbrains.annotations.NotNull HttpServletResponse response, @NotNull FilterChain filterChain) throws ServletException, IOException {
        ...
        log.debug("客戶端攜带的JWT：{}", jwt);

        // 判斷JWT的基本有效性（没有必要嘗試解析格式明顯錯誤的JWT資料）
        if (!StringUtils.hasText(jwt) || jwt.length() < JWT_MIN_LENGTH) {
            // 對於無效的JWT，應該直接放行
            log.warn("當前請求中，客戶端没有攜带有效的JWT，將放行");
            filterChain.doFilter(request, response);
            return;
        }
        ...
        try {
            ...
            String message = "非法訪問，你的本次操作已經被紀錄！";
        ...
        // 判斷緩存中是否存在資料
        if (userState == null) {
            // 放行，不會向SecurityContext中存入認證信息，則相當於没有攜带JWT
