package cn.tedu.meimall.attachment.filter;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter implements HttpConsts, JwtConsts {
    ...
    @Override
    protected void doFilterInternal(HttpServletRequest request, @NonNull HttpServletResponse response,@NotNull FilterChain filterChain) throws ServletException, IOException {
        ...
        if (!StringUtils.hasText(jwt) || jwt.length() < JWT_MIN_LENGTH) {
            // 對於無效的JWT，應該直接放行
            log.warn("當前請求中，客戶端没有攜帶有效的JWT，將放行");
        ...
        try {
            ...
            String message = "服務器忙，請稍後再试！【看到這句時，你應該檢查服務器端的控制台，並在JwtAuthorizationFilter中解析JWT時添加處理異常的catch程式碼】";
        ...
        // 判斷緩存中是否存在資料
        if (userState == null) {
            // 放行，不會向SecurityContext中存入認證信息，則相當於没有攜帶JWT
