...
import cn.tedu.meimall.admin.mall.dao.persist.mapper.CheckLogMapper;
...
public class CheckLogRepositoryImpl implements ICheckLogRepository {
    ...
    public int insert(CheckLog checkLog) {
        log.debug("開始執行【插入審核日誌資料】的資料訪問,參數:{}",checkLog);
    ...
    public int deleteByResource(Integer resourceType, Long resourceId) {
        log.debug("開始執行【根據資源刪除審核日誌】的資料訪問,資源類型:{},資源ID:{}", resourceType, resourceId);
    ...
    @Override
    public PageData<CheckLogListItemVO> listByResourceType(int resourceType, int pageNum, int pageSize) {
